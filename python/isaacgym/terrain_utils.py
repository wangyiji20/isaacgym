...
def random_uniform_terrain(terrain, min_height, max_height, step=1, downsampled_scale=None,):
    ...
    z_upsampled=z_upsampled.astype(np.int16)
    terrain.height_field_raw += z_upsampled
    
    std1=np.std(z_upsampled[int(terrain.width/4),0:terrain.length])
    std2=np.std(z_upsampled[int(terrain.width/2),0:terrain.length])
    std3=np.std(z_upsampled[int(3*terrain.width/4),0:terrain.length])
    terrain.std=(std1+std2+std3)/3
    return terrain
...
def discrete_obstacles_terrain(terrain, max_height, min_size, max_size, num_rects, platform_size=1.):
    ...
    width_range = range(min_size, max_size, 2)
    length_range = range(min_size, max_size, 2)

    for _ in range(num_rects):
        width = np.random.choice(width_range)
        length = np.random.choice(length_range)
        start_i = np.random.choice(range(0, i-width, 2))
        start_j = np.random.choice(range(0, j-length, 2))
    ...
    std1=np.std(terrain.height_field_raw[0:terrain.width,int(terrain.length/4)])
    std2=np.std(terrain.height_field_raw[0:terrain.width,int(terrain.length/2)])
    std3=np.std(terrain.height_field_raw[0:terrain.width,int(3*terrain.length/4)])
    terrain.std=(std1+std2+std3)/3
    return terrain
...
def pyramid_stairs_terrain(terrain, step_width, step_height, platform_size=1.):
    ...
    # calculate roughness    
    linear=np.linspace(0,height,start_x+1)
    offset=terrain.height_field_raw[0:start_x+1,int(terrain.width/2)]-linear
    terrain.std=np.std(offset)
    return terrain
